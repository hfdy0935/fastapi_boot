...
from fastapi_boot.exception.bean import (
    BeanDecoratedFunctionHasNoReturnAnnotationException,
    BeanUsePositionException,
    ComponentUsePositionException,
    NotSupportFunctionBeanException,
)
from fastapi_boot.exception.route import (
    ClassWithSpecificDecoratorException,
    RequestMethodNotFoundException,
    UnExpectedWebsocketInRequestMappingException,
)
from fastapi_boot.model.route_model import Symbol


def must_decorate_function(
    Raise: Type[Exception], msg: str = "", show_pos: bool = True
):
    """base
...
@must_decorate_function(
    ClassWithSpecificDecoratorException, "specific mapping can only decorate function"
)
def validate_specific_mapping(obj: Callable): ...


@must_decorate_class("Prefix can only decorate class")
...
@must_decorate_class("Controller can only decorate class")
...
def validate_request_mapping(obj: Callable, methods: List[str], symbol: Symbol):
    ...
    if inspect.isclass(obj):
        raise Exception(
            f"RequestMapping must decorate function, position: {symbol.pos}"
        )
