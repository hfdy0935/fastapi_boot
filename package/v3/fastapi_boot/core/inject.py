...
def find_dependency_once(
    ...
    if inject_type == InjectType.NAME:
        res = application.sa.inject_by_name(name)
...
def find_dependency(
    ...
    """寻找依赖"""
    res: T | None = None
    sa = GlobalVar.get_app(stack_path).sa
    timeout_second = sa.scan_timeout_second
    # 开始计时，装配
    start = time.time()
    while True:
        print("\r", end="")
        if inject_type == InjectType.NAME:
            msg = f"名为 {name} "
            res = find_dependency_once(InjectType.NAME, stack_path, name=name)
        elif inject_type == InjectType.TYPE:
            msg = f"类型为 {DepType.__name__ if DepType else ...}"
            res = find_dependency_once(InjectType.TYPE, stack_path, DepType=DepType)

        elif inject_type == InjectType.NAME_OF_TYPE:
            msg = f"类型为 {dep_type_name}"
            res = find_dependency_once(InjectType.NAME_OF_TYPE, stack_path, dep_type_name=dep_type_name)
        if res:
            return res
        time.sleep(0.1)
...
def resolve_at(self: type["Inject"], other: type[T], stack_path: str):
    ...
    if self.INJECT_TYPE == InjectType.TYPE:
        ...
        return find_dependency(InjectType.NAME, stack_path, name=self.INJECT_NAME)
