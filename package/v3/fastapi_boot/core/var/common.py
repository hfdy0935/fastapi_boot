from collections.abc import Callable
from fastapi_boot.utils.judger import is_ancestor


class CommonVar:
    # 应用列表
    __application_list: list = []
    # 挂载之后要运行的任务，key是应用在项目中的路径，value是任务列表
    __mounted_task_list: dict[str, list[Callable]] = {}

    # -------------------------------------------------------- app ------------------------------------------------------- #

    @staticmethod
    def get_app(path: str):
        """根据路径获取application

        Args:
            path (str): 路径

        Returns:
            Optional[MainApplication]: 可能找不到
        """
        for app in CommonVar.__application_list:
            if app.isin(path):
                return app

    @staticmethod
    def get_all_apps():
        """获取所有application

        Returns:
            list[MainApplication]: 列表
        """
        return CommonVar.__application_list

    @staticmethod
    def add_app(application):
        """添加一个application"""
        CommonVar.__application_list.append(application)

    # ------------------------------------------------------- task ------------------------------------------------------- #

    @staticmethod
    def add_task(path: str, task: Callable):
        """添加任务"""
        ls = CommonVar.__mounted_task_list.get(path, [])
        if len(ls) == 0:
            CommonVar.__mounted_task_list[path] = [task]
        else:
            CommonVar.__mounted_task_list.update({path: [*ls, task]})

    @staticmethod
    def run_task(app_path: str):
        """根据app所在位置，运行该位置下所有task"""
        for path, task_list in CommonVar.__mounted_task_list.items():
            if is_ancestor(app_path, path):
                map(lambda task: task(), task_list)
                # 清空任务列表
                task_list.clear()
